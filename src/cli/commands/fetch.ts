...
export async function fetchCandles(args: FetchArgs): Promise<number> {
  ...
  try {
    ...
    const sourceKind = args.source ?? 'kite';

    let source: HistoricalSource;
    if (sourceKind === 'yahoo') {
      source = new YahooSource({ exchangeSuffix: exchange === 'BSE' ? '.BO' : '.NS' });
    } else {
      const env = parseEnv();
      ...
      source = new KiteSource({ kite: client, instruments: instrumentStore, exchange });
    }
